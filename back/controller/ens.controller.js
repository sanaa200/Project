...
const { connect } = require('../router/ens.router');
...
module.exports.signup = (req, res) => {
    ...
    connection.query(emailchkqry, async (err, result) => {
        ...
        if (result.length > 0) {
            ...
            let insertqry = `insert into enseignant(ADM_ID,NOM,PRENOM,EMAIL,PASSWORD,ROLE) values('${id}','${nom}','${prenom}','${email}','${decryptpwd}','${role}')`;
...
module.exports.login = (req, res) => {
    ...
    // checkemailid
    let chkemailid = ` select * from enseignant where email ='${email}' `;
        ...
        if (result.length > 0) {
            ...
            if (chkpwd == true) {
                ...
                res.send({
                    ...
                    token : token , 
                    result : data ,
                    message: 'enseignant login successful !!! '
                });
            }
            else {
                res.send({
                    status: false,
                    message: ' invalid enseignant !!! '
                });
            }
